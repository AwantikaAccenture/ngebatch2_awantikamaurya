...
//Arr
   ...
   //Assert
     ...
     expect(mockedGet.mock.calls[0][0]).toEqual(`${myServer}deck/${deckID}/draw/?count=${numberofCards}`
)
    })
  })
  ...
  describe('when using remainingCards', () => {
    test('it will return the number of remaining cards', async () => {
      ...
      expect(mockedGet.mock.calls[0][0]).toEqual(`${myServer}deck/${deckID}/shuffle/?remaining=true`)
    })
  })
