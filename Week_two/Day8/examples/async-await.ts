...
const waitThreeAsync = async (): Promise<number> => {
  const a = await waitOne() // 1The await keyword blocks until the promise resolves
  const b = await waitTwo(a) //2 So we can use the result of the first function call as an argument to the next one
  const c = await waitThree(b)//4
  return a+b+c; 
}
