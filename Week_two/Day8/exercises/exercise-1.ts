...
new Promise((resolve)=>{  const result = userData.filter((user) => user.name === personName)[ 0 ] || null
  resolve(result)
})
  //
...
new Promise((resolve,reject)=>{const result = foodData.filter((food) => food.id === foodId)[ 0 ] || null
  resolve(result)
})
