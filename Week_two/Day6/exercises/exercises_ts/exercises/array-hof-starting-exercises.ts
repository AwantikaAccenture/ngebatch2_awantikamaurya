...
const shoutOut = academitePets.map(pet =>`HELLO ${pet.toUpperCase()}`); 'TODO'
...
const above12Months = academitePetsWithAges.filter(pet => pet.age>=12);'TODO'
...
const filteredAndShouted = academitePetsWithAges.filter(pet => pet.age<12); 'TODO'
