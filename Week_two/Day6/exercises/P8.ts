...
//   }
 ...
 const queryUser = (personName: string):Promise<User> => new Promise((resolve) =>{
 const result = userData.filter((name) =>name.name === personName)[0] ||null;
 resolve(result); })
//     const result = userData.filter((user) => user.name === personName)[ 0 ] || null
//     callback(result)
  ...
  // When you make a new promise you will need to implicitly or explicitly return it
  const queryFood = (foodId: number): Promise<Food> =>new Promise((resolve)=>{
  ...
  // When you make a new promise you will need to implicitly or explicitly return it
  const findFavouriteFood = (name: string) => new Promise((resolve) =>{
    queryUser(name).then((person) =>queryFood(person.food))
